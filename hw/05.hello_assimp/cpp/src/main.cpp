...
void write_vector(std::ofstream& out, const glm::vec3& vec);
// TODO 장면 저장함수
void save_scene_txt();
////////////////////////////////////////////////////////////////////////////////
...
void save_scene_txt() {
    ...
    for (const auto& obj : g_objects) {
        ...
        write_vector(out, obj.get_scale());     // scale
        write_vector(out, obj.get_translate()); // translate
    }

    // 카메라 정보 추출
    out << g_cameras.size() << '\n'; // 생성할 카메라의 갯수
    for (const auto& cam : g_cameras) {
        write_vector(out, cam.position()); // position

        // at_position (position + front_direction)
        write_vector(out, cam.position() + cam.front_direction());

        write_vector(out, cam.up_direction()); // up_direction
    }
...
void render_scene()
{
  ...
  for (std::size_t i = 0; i < g_objects.size(); ++i)
  {
    ...
    mat_PVM = mat_proj * mat_view * g_objects[i].get_model_matrix();
