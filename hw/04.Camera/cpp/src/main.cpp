...
{
    // TODO
    float fov = g_camera.fovy();
    float ortho = g_camera.ortho_scale();

    if (g_camera.mode() == Camera::kPerspective) {
        fov -= (float)y;

        fov = (fov < 10.0f) ? 10.0f : (fov > 160.0f) ? 160.0f : fov;

        g_camera.set_fovy(fov);
    } else {
        ortho -= (float)y;

        ortho = (ortho < 0.1f) ? 0.1f : (ortho > 10.0f) ? 10.0f : ortho;

        g_camera.set_ortho_scale(ortho);
    }
}
