...
void Mesh::set_gl_position_buffer_()
{
    ...
    for (unsigned int i = 0; i < tv_indices_.size(); ++i)
    {
        ...
        glm::vec3 position(pmesh_->mVertices[vertexIndex].x, pmesh_->mVertices[vertexIndex].y, pmesh_->mVertices[vertexIndex].z);
...
void Mesh::set_gl_color_buffer_(unsigned int cs_idx)
{
    ...
    {
        unsigned int vertexIndex = tv_indices_[i];
        glm::vec3 color(pmesh_->mColors[cs_idx][vertexIndex].r, pmesh_->mColors[cs_idx][vertexIndex].g, pmesh_->mColors[cs_idx][vertexIndex].b);
        tv_colors.push_back(color);
    }
...
void Mesh::set_gl_normal_buffer_(ShadingType shading_type)
{
    ...
    {
        unsigned int index0 = tv_indices_[i];
        unsigned int index1 = tv_indices_[i + 1];
        unsigned int index2 = tv_indices_[i + 2];

        glm::vec3 vertex0(pmesh_->mVertices[index0].x, pmesh_->mVertices[index0].y, pmesh_->mVertices[index0].z);
        glm::vec3 vertex1(pmesh_->mVertices[index1].x, pmesh_->mVertices[index1].y, pmesh_->mVertices[index1].z);
        glm::vec3 vertex2(pmesh_->mVertices[index2].x, pmesh_->mVertices[index2].y, pmesh_->mVertices[index2].z);
        ...
        // Add up the per-triangle normal to vertex normals
        v_smooth_normals[index0] += normal;
        v_smooth_normals[index1] += normal;
        v_smooth_normals[index2] += normal;
    }
    ...
    {
        unsigned int vertexIndex = tv_indices_[i];
        tv_smooth_normals[i] = v_smooth_normals[vertexIndex];
    }
