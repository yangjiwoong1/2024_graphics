...
void render_object()
{
  ...
  glUniform3fv(loc_u_camera_position, 1, glm::value_ptr(g_cameras[g_cam_select_idx].position()));
  ...
  for (std::size_t i = 0; i < g_models.size(); ++i)
  {
    ...
    mat_model = glm::mat3(g_models[i].get_model_matrix());
    glm::mat3 mat_normal = glm::mat3(mat_model); 
    glm::mat4 mat_PVM = mat_proj * mat_view * g_models[i].get_model_matrix(); 
    ...
    g_models[i].draw(loc_a_position, loc_a_normal, loc_u_obj_ambient, loc_u_obj_diffuse, loc_u_obj_specular, loc_u_obj_shininess);
